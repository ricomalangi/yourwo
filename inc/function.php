<?php
include ('connect.php');
function genCode($field, $table,$paket, $char){ //kode awal, field kode, nama table dan panjang kode
	$thn= date("Ym");
	$first = $paket.$thn;
	$get = mysqli_query($konek,"SELECT MAX(RIGHT($field, $char)) as maxID FROM $table where left(id_card,1)='$paket' ORDER BY $field");
	$code = mysqli_fetch_array($get);
	$genKode = $code['maxID'];
...
function query($qry) {
	global $connect;
	$result = mysqli_query($connect,$qry) or die("Gagal melakukan query pada :
	 <br>$qry<br><br>Kode Salah : <br>&nbsp;&nbsp;&nbsp;" . mysqli_error() . "!");
	return $result;
}
...
function uang($angka)
{
echo "Rp." . number_format($angka, 0, "", ".") . ",00";
}
...
function combo_dppu($kode) {
	echo "<option value='' selected>PILIH DPPU</option>";
	$query = query("SELECT *  FROM table_dppu where region='I'");
	while ($row = mysqli_fetch_row($query)) {
		if ($kode == "")
			echo "<option value='$row[0]'> " . ucwords($row[2]) . " </option>";
		else
			echo "<option value='$row[0]'" . selected($row[0], $kode) . "> " . ucwords($row[2]) . " </option>";
	}
}
...
$BulanIn =array ("Januari", "Februari", "Maret", "April", "Mei","Juni","Juli","Agustus","September","Oktober","November","Desember");
$tahun = substr($date,0,4);
$bulan = substr($date,5,2);
$tgl   = substr($date,8,2);
$result = $tgl . " " . $BulanIn[(int)$bulan-1] . " " . $tahun;
return($result);
