...
function uploadGambar($image, $base_url, $action, $id_user = null){
    ...
    if($action == 'create'){
        if(!in_array($ekstensiGambar, $validEktensi)){
            header("Location: $base_url"."/users/add-user/message/error-image");
            die;
        }
        if($ukuranFile > 2000000){
            header("Location: $base_url"."/users/add-user/message/file-to-large");
            die;
        }
    }
    if($action == 'update'){
        if(!in_array($ekstensiGambar, $validEktensi)){
            header("Location: $base_url"."/users/edit-user/$id_user/message/error-image");
            die;
        }
        if($ukuranFile > 2000000){
            header("Location: $base_url"."/users/edit-user/$id_user/message/file-to-large");
            die;
        }
    }
    ...
    move_uploaded_file($tmpName, '../assets/img/avatars/'. $namaFileBaru);
    return $namaFileBaru;
}
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create'])){
    ...
    if($result){
        header("Location: $base_url"."/users/message/success");
    } else{
        header("Location: $base_url"."/users/message/failed");
    }
}
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update'])){
    ...
    $result = mysqli_query($connect,$sql) or die(mysqli_error($connect));
    if($result){
        header("Location: $base_url/users/edit-user/$id_user/message/success");
    } else{
        header("Location: $base_url/users/edit-user/$id_user/message/failed");
    }
}